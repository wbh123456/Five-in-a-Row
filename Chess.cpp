...
int winning_point(std::vector<int> & moves, bool first); //give a vector with 11 int, return the winning point, bool represents if ai is first
...
int Chess::winning_point_gain(int x, int y) {
	...
	point_before += winning_point(move_block, first);
	move_block[(MOVEBLOCK_SIZE - 1) / 2] = first ? 1 : 2;
	point_after += winning_point(move_block, first);
	...
	points_gain = point_after - point_before;
	

	//FOR DEGUUGING: display points gain at x, y


	gaining_vector[x][y][1] = point_after;
	gaining_vector[x][y][0] = point_before;
...
//give a vector with 11 int, return the winning point, bool represents if ai is first
int winning_point(std::vector<int>& moves, bool first) {
	/*
	winning points is based on (base score) and (multi factor)
	-base score depends on num of self repeitations(how many in a row)
	...
	*/
	...
	for (int self = 1; self != 3; self++) {
		...
		int middle_plus_1= ((MOVEBLOCK_SIZE - 3) / 2);
		for (int i = middle_plus_1; i != -1; i--) {
			if (moves[i] != self && moves[i] != 0) {
				min_bound = i;
				break;
			}
			else if (i == 0) min_bound = -10;
		}
		int middle_less_1 = ((MOVEBLOCK_SIZE - 3) / 2);
		for (int i = middle_less_1; i != MOVEBLOCK_SIZE; i++) {
			if (moves[i] != self && moves[i] != 0) {
				max_bound = i;
				break;
			}
			else if (i == MOVEBLOCK_SIZE-1) max_bound = 30;
		}
		...
		for (int i = index_bound1; i != index_bound2 +1; i++) {
			...
			if (moves[i] == self) {
				...
				int alone_dis = bound_in_middle?  min(min(i - min_bound, max_bound - i), std::abs(6 - i)) : min(i - min_bound, max_bound - i);
			...
			//if rep stops (hit bounds), then sum up the scroes
				...
				else if (rep == 2) rep_point = 50;
				else if (rep == 3) rep_point = 500;
				else if (rep == 4) rep_point = 5000;
